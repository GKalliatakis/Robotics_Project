...
import roslib; roslib.load_manifest('test_move_goal')
import rospy
import smach
...
class Patrol():
	...
	self.humanDetect_pub.publish("Start")
	rospy.sleep(5)
	if self.humans > 0:
	    rospy.loginfo(str(self.humans) + " humans detected")
	else:
            rospy.loginfo("Nobody here.")
	self.humanDetect_pub.publish("Stop")
                
    def detectionCallback(self,data):
        self.humans = int(str(data.data))    
